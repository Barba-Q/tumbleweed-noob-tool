...
if [ "$ask" == "01 Install common software" ]; then
	...
	[[ -z "$software" ]] 
	then {
	echo nothing selected, exiting && exit 0
	}
	fi
...
#		fi	
	...
	#	INSTALLATION
	####################################################
	PASSWORD=`zenity --title='Ill install what Ive found' --hide-text --timeout=10 --text='You have to reenter your password during the process, this is intended.\n\nRoot password:' --entry`
	echo $PASSWORD | \
			sudo -S killall packagekitd 
			sudo -S zypper ar http://download.nvidia.com/opensuse/tumbleweed/  NVIDIA
			sudo -S zypper ar http://ftp.gwdg.de/pub/linux/packman/suse/openSUSE_Tumbleweed/ Multimedia
...
if [ "$ask" == "04 Perform Software Upgrade" ]; then
	####################################################
	PASSWORD=`zenity --title='Perform software upgrade' --hide-text --text='You have to reenter your password during the process, this is intended.\n\nRoot password:' --entry`
	echo $PASSWORD | \
			sudo -S killall packagekitd 
			sudo -S zypper ar http://download.nvidia.com/opensuse/tumbleweed/  NVIDIA
			sudo -S zypper ar http://ftp.gwdg.de/pub/linux/packman/suse/openSUSE_Tumbleweed/ Multimedia
