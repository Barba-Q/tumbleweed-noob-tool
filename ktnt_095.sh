...
if [ "$ask" == "04 Perform Software Upgrade" ]; then
	...
	zenity --info --title='Let me see..' --width=450 --height=100 --text='You want me to upgrade your system?\n(Note that in some cases you have to help me with some comflict decisions.)' 
			...
			sudo -S zypper --no-gpg-checks --non-interactive ar http://ftp.gwdg.de/pub/linux/packman/suse/openSUSE_Tumbleweed/ Multimedia;
			sudo -S zypper ref;
			sudo -S zypper dup -l --allow-vendor-change"
                if 
                    [ $? -eq 1 ]
                    then {
                    zenity --info --width=450 --height=200 --text "Somethings wrong with zypper, is any other softwaremanager running?" && bash debug: tnt_08.sh && exit 0
...
if [ "$ask" == "05 One click set up" ]; then
	####################################################
	zenity --info --title='Let me see..' --width=450 --height=200 --text='I´ll perform automatic set up now!\nSee readme for further information\n\nI´ll reboot your system when I´m done, so save your stuff and pray ;)' 
			xterm -e "sudo -S killall packagekitd; 
			sudo -S zypper --no-gpg-checks --non-interactive ar http://download.nvidia.com/opensuse/tumbleweed/  NVIDIA;
			sudo -S zypper --no-gpg-checks --non-interactive ar http://ftp.gwdg.de/pub/linux/packman/suse/openSUSE_Tumbleweed/ Multimedia;
			sudo -S zypper ref;
			sudo -S zypper in -l -y --allow-vendor-change audacity chromium discord obs-studio vlc vlc-codecs steam glibc-locale-base-32bit libavcodec58 libavcodec57 lame speex xvidcore h264enc ffmpeg-4 ffx264 ffxvid gstreamer-plugins-bad gstreamer-plugins-good-extra gstreamer-plugins-libav fetchmsttfonts wine youtube-dl
