...
--text=" "	\
--column="How can I be of service?"	\
...
if [ "$ask" == "01 Install common software" ]; then
	...
	--text " "	\
	--column "What do you want? Multiple selections possible" --column "Software" --column "Description"	\
		False Audacity    "Non-linear Audio editor"	\
		False Blender     "3D animation suite, Video editor"	\
		...
		False VLC         "Allround Mediaplayer - installs codecs"	\
		False Wine        "Run software for MS Windows on Linux"	\
		False YoutubeDL   "Downlod videos and playlists from Websites")
...
if [ "$ask" == "02 Install tools and drivers" ]; then
    tools=$(zenity --list --checklist --width=600 --height=400	\
	--title "Tumbleweed Noob Tool - en"	\
	...
	if [[ $tools = *codecs* ]]
	then {
	t_b="libavcodec57 lame libxvidcore4 h264enc ffmpeg-4 gstreamer-plugins-bad gstreamer-plugins-bad-orig-addon gstreamer-plugins-good-extra gstreamer-plugins-libav gstreamer-plugins-ugly"
	}
	fi
...
if [ "$ask" == "04 Perform Software Upgrade" ]; then
	...
	[ $? -eq 0 ]
	then {
	zenity --info --width=450 --height=100 --text "Seems like everything is done"
	exit 0
	}
	zenity --info --width=450 --height=100 --text "Something went wrong. Please startover and try again." 1>&2
	exit 0
...
#   SELECTION 05 AUTOMATIC
####################################################
if [ "$ask" == "05 One click set up" ]; then
	####################################################
	zenity --info --title='Let me see..' --width=450 --height=200 --text='I´ll perform automatic set up now!\nSee readme for further information\n'
			xterm -e "sudo -S killall packagekitd;
			sudo -S zypper --no-gpg-checks --non-interactive ar http://download.nvidia.com/opensuse/tumbleweed/  NVIDIA;
			sudo -S zypper --no-gpg-checks --non-interactive ar http://ftp.gwdg.de/pub/linux/packman/suse/openSUSE_Tumbleweed/ Multimedia;
			sudo -S zypper ref;
			sudo -S zypper in -l -y --allow-vendor-change audacity chromium discord obs-studio vlc vlc-codecs steam glibc-locale-base-32bit fetchmsttfonts wine youtube-dl libavcodec57 lame libxvidcore4 h264enc ffmpeg-4 gstreamer-plugins-bad gstreamer-plugins-bad-orig-addon gstreamer-plugins-good-extra gstreamer-plugins-libav gstreamer-plugins-ugly
			sudo zypper dup -y --force-resolution --allow-vendor-change"
                if
                    [ $? -eq 1 ]
                    then {
